document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateQuickInfo(room) {
    try {
      ...
      if (countEl && room.playerCount !== undefined) {
        countEl.innerHTML =
          '<strong>' + I18n.t('common.total_players') + ':</strong> ' + room.playerCount;
      }
    } catch (e) {}
  }
  var defaultAvatar =
    (window.SERVER_CONFIG && window.SERVER_CONFIG.defaultAvatar) ||
    'https://phira.5wyxi.com/files/6ad662de-b505-4725-a7ef-72d65f32b404';
  ...
  function showAnnouncement(m) {
    try {
      ...
      switch (m.type) {
        case 'Chat':
          content = uName + ': ' + m.content;
          break;
        case 'JoinRoom':
          content = I18n.t('room.events.join', { user: m.name || uName });
          break;
        case 'LeaveRoom':
          content = I18n.t('room.events.leave', { user: m.name || uName });
          break;
        case 'CreateRoom':
          content = I18n.t('room.events.create', { user: uName });
          break;
        case 'NewHost':
          content = I18n.t('room.events.new_host', { user: uName });
          break;
        case 'SelectChart':
          content = I18n.t('room.events.select_chart', { name: m.name, id: m.id });
          break;
        case 'GameStart':
          content = I18n.t('room.events.game_start');
          break;
        case 'Ready':
          content = I18n.t('room.events.ready', { user: uName });
          break;
        case 'CancelReady':
          content = I18n.t('room.events.cancel_ready', { user: uName });
          break;
        case 'StartPlaying':
          content = I18n.t('room.events.start_playing');
          break;
        case 'Played':
          ...
          break;
        case 'Abort':
          content = I18n.t('room.events.abort', { user: uName });
          break;
        case 'GameEnd':
          content = I18n.t('room.events.game_end');
          break;
        case 'LockRoom':
          content = I18n.t('room.events.lock_room', {
            status: m.lock ? I18n.t('room.events.lock') : I18n.t('room.events.unlock'),
          });
          break;
        case 'CycleRoom':
          content = I18n.t('room.events.cycle_room', {
            status: m.cycle ? I18n.t('room.events.on') : I18n.t('room.events.off'),
          });
          break;
        default:
    ...
    try {
      ...
      if (action === 'message') {
        ...
        await fetch('/api/admin/set-room-blacklist', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ roomId: roomId, userIds: ids }),
        });
        ...
        await fetch('/api/admin/set-room-whitelist', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ roomId: roomId, userIds: ids2 }),
        });
      }
      rst();
    } catch (e) {
  ...
  window.openAdminModal = function (title, bodyHtml, confirmAction) {
    ...
    btn.onclick = function () {
      confirmAction();
      document.getElementById('admin-inline-modal').style.display = 'none';
    };
  ...
  window.showBlacklistModal = async function (type) {
    ...
    var list = data[type] || [];
    var html =
      '<div style="margin-bottom:12px;"><span style="font-size:0.78rem;color:var(--text-secondary);">Current ' +
      type +
      ':</span><div style="color:var(--text);font-size:0.85rem;margin-top:4px;">' +
      (list.length ? list.join(', ') : '<em>Empty</em>') +
      '</div></div>';
    html +=
      '<label style="font-size:0.78rem;color:var(--text-secondary);display:block;margin-bottom:4px;">Set ' +
      type +
      ' (comma-separated IDs):</label>';
    html +=
      '<input type="text" id="admin-input-value" style="width:100%;padding:8px 10px;background:var(--bg);border:1px solid var(--border);color:var(--text);font-size:0.85rem;font-family:var(--font-sans);" value="' +
      list.join(',') +
      '">';
  ...
  function updateTotalPlayers(count) {
    ...
    if (isAdmin) {
      totalPlayersDiv.innerHTML =
        '<a href=\"/players.html\">' +
        content +
        '</a><a href=\"/logout\" class=\"logout-icon\" title=\"Logout\">Logout</a>';
    } else {
  ...
  function renderRoomDetails(details) {
    try {
      ...
      if (details.otherRooms) {
        var newIds = new Set(
          details.otherRooms.map(function (r) {
            return r.id;
          }),
        );
        var currentIds = new Set(
          currentOtherRooms.map(function (r) {
            return r.id;
          }),
        );
        ...
        if (needsUpdate || currentOtherRooms.length === 0) {
          ...
          currentOtherRooms = currentOtherRooms.map(function (cr) {
            return (
              details.otherRooms.find(function (r) {
                return r.id === cr.id;
              }) || cr
            );
          });
      ...
      var chartInfoHtml = [
        ...
        details.selectedChart && details.selectedChart.illustration
          ? '<div class="chart-illustration"><img src="' +
            details.selectedChart.illustration +
            '" alt="Illustration"></div>'
          : '<div class="chart-illustration" style="background:var(--bg-secondary); height:200px; display:flex; align-items:center; justify-content:center; color:var(--text-tertiary); border-radius:var(--radius);">' +
            I18n.t('room.no_illustration') +
            '</div>',
        '<div class="chart-details-box">',
        '<h4>' + I18n.t('room.chart_info') + '</h4>',
        '<p><strong>Name:</strong> <span style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:180px;" title="' +
          (details.selectedChart ? details.selectedChart.name : '') +
          '">' +
          (details.selectedChart ? details.selectedChart.name : I18n.t('room.not_selected')) +
          '</span></p>',
        '<p><strong>' +
          I18n.t('room.id') +
          ':</strong> ' +
          (details.selectedChart ? details.selectedChart.id : 'N/A') +
          '</p>',
        '<p><strong>' +
          I18n.t('room.level') +
          ':</strong> ' +
          (details.selectedChart ? details.selectedChart.level : 'N/A') +
          '</p>',
        '<p><strong>' +
          I18n.t('room.difficulty') +
          ':</strong> ' +
          (details.selectedChart ? details.selectedChart.difficulty : 'N/A') +
          '</p>',
        '<p><strong>' +
          I18n.t('room.charter') +
          ':</strong> <span style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:180px;" title="' +
          (details.selectedChart ? details.selectedChart.charter : '') +
          '">' +
          (details.selectedChart ? details.selectedChart.charter : 'N/A') +
          '</span></p>',
        '<p><strong>' +
          I18n.t('room.composer') +
          ':</strong> <span style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:180px;" title="' +
          (details.selectedChart ? details.selectedChart.composer : '') +
          '">' +
          (details.selectedChart ? details.selectedChart.composer : 'N/A') +
          '</span></p>',
        '<p><strong>' +
          I18n.t('room.rating') +
          ':</strong> <span>' +
          starsHtml +
          ' <span style="font-size:0.9em; color:var(--text-tertiary);">(' +
          ratingDisplay +
          ' / 5.00)</span></span></p>',
        '</div>',
        '</div>',
      ...
      // Host Info
      var host = (details.players || []).find(function (p) {
        return p.id === details.ownerId;
      });
      var hostHtml = [
        '<div class="detail-card">',
        '<h3>' + I18n.t('room.host_info') + '</h3>',
        '<div class="uploader-info">',
        '<a href="https://phira.moe/user/' +
          details.ownerId +
          '" target="_blank" style="text-decoration:none; color:inherit;">',
        '<img src="' +
          (host ? host.avatar : defaultAvatar) +
          '" class="uploader-avatar" alt="Host avatar">',
        ...
        '<p style="font-size: 0.8rem; color: var(--text-tertiary); margin-top: 8px; font-weight:700;">' +
          I18n.t('room.room_host_tag') +
          '</p>',
        '</div>',
        '</a>',
        '</div>',
      ...
      var playersHtml = sortedPlayers
        .map(function (p) {
          var isOwner = p.id === details.ownerId;
          var isServer = p.id === -1;
          var statusClass = isServer
            ? 'status-bot'
            : p.isReady
              ? 'status-ready'
              : 'status-not-ready';
          ...
          return [
            ...
            '<img src="' +
              ...
              '" class="player-avatar-small" alt="' +
              (p.name || 'Player') +
              '">',
            '<a class="player-name ' +
              nameClass +
              '" href="' +
              (isServer ? '#' : 'https://phira.moe/user/' + p.id) +
              '" target="_blank"><span class="name-prefix ' +
              prefixClass +
              '">' +
              prefixText +
              '</span>' +
              (p.name || I18n.t('common.unknown')) +
              '</a>',
            '</div>',
            '<span class="player-status ' +
              statusClass +
              '">' +
              (isServer ? 'Bot' : p.isReady ? 'Ready' : 'Not Ready') +
              '</span>',
            '</li>',
          ].join('');
        })
      ...
      var resultsTableBody =
        scores.length > 0
          ? scores
              .map(function (p, i) {
                return [
                  ...
                  '<td style="text-align:left; max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">' +
                    ...
                    '</td>',
                  '<td class="score-val">' +
                    ((p.score ? p.score.score : 0) || 0).toLocaleString() +
                    '</td>',
                  '<td>' + (((p.score ? p.score.accuracy : 0) || 0) * 100).toFixed(2) + '%</td>',
                  '<td style="font-weight: 700; color: var(--primary);">' +
                    ((p.score ? p.score.maxCombo : 0) || 0) +
                    '</td>',
              ...
              .join('')
          : '<tr><td colspan="6" style="padding: 40px; color: var(--text-tertiary); font-style: italic;">' +
            I18n.t('room.no_results') +
            '</td></tr>';
      ...
      // Messages
      var messagesHtml = (details.messages || [])
        .map(function (m) {
          var text = '';
          var uName = m.userName || I18n.t('common.unknown');
          switch (m.type) {
            case 'Chat':
              text =
                '<span class="msg-user">' +
                uName +
                ':</span><span class="msg-chat">' +
                m.content +
                '</span>';
              break;
            case 'CreateRoom':
              text =
                '<span class="msg-system">' +
                I18n.t('room.events.create', { user: uName }) +
                '</span>';
              break;
            case 'JoinRoom':
              text =
                '<span class="msg-system">' +
                I18n.t('room.events.join', { user: m.name || uName }) +
                '</span>';
              break;
            case 'LeaveRoom':
              text =
                '<span class="msg-system">' +
                I18n.t('room.events.leave', { user: m.name || uName }) +
                '</span>';
              break;
            case 'NewHost':
              text =
                '<span class="msg-system">' +
                I18n.t('room.events.new_host', { user: uName }) +
                '</span>';
              break;
            case 'SelectChart':
              text =
                '<span class="msg-system">' +
                I18n.t('room.events.select_chart', { name: m.name, id: m.id }) +
                '</span>';
              break;
            case 'GameStart':
              text = '<span class="msg-system">' + I18n.t('room.events.game_start') + '</span>';
              break;
            case 'Ready':
              text =
                '<span class="msg-ready">' +
                I18n.t('room.events.ready', { user: uName }) +
                '</span>';
              break;
            case 'CancelReady':
              text =
                '<span class="msg-system">' +
                I18n.t('room.events.cancel_ready', { user: uName }) +
                '</span>';
              break;
            case 'CancelGame':
              text =
                '<span class="msg-system">' +
                I18n.t('room.events.cancel_game', { user: uName }) +
                '</span>';
              break;
            case 'StartPlaying':
              text = '<span class="msg-playing">' + I18n.t('room.events.start_playing') + '</span>';
              break;
            case 'Played':
              text =
                '<span class="msg-system">' +
                I18n.t('room.events.played', {
                  user: uName,
                  score: (m.score || 0).toLocaleString(),
                  acc: ((m.accuracy || 0) * 100).toFixed(2),
                }) +
              ...
              break;
            case 'GameEnd':
              text = '<span class="msg-system">' + I18n.t('room.events.game_end') + '</span>';
              break;
            case 'Abort':
              text =
                '<span class="msg-system">' +
                I18n.t('room.events.abort', { user: uName }) +
                '</span>';
              break;
            case 'LockRoom':
              text =
                '<span class="msg-system">' +
                I18n.t('room.events.lock_room', {
                  status: m.lock ? I18n.t('room.events.lock') : I18n.t('room.events.unlock'),
                }) +
                '</span>';
              break;
            case 'CycleRoom':
              text =
                '<span class="msg-system">' +
                I18n.t('room.events.cycle_room', {
                  status: m.cycle ? I18n.t('room.events.on') : I18n.t('room.events.off'),
                }) +
                '</span>';
              break;
            default:
              text = '<span class="msg-system">' + m.type + ' event</span>';
          }
      ...
      if (isAdmin) {
        adminHtml = [
          ...
          '<button class="admin-btn action-primary" onclick="window.adminAction(\'force-start\')">' +
            I18n.t('room.admin.start') +
            '</button>',
          '<button class="admin-btn" onclick="window.adminAction(\'toggle-lock\')">' +
            I18n.t('room.admin.lock') +
            '</button>',
          '<button class="admin-btn" onclick="window.adminAction(\'toggle-mode\')">' +
            I18n.t('room.admin.mode') +
            '</button>',
          '<button class="admin-btn action-danger" onclick="window.adminAction(\'close\')">' +
            I18n.t('room.admin.close') +
            '</button>',
          '<button class="admin-btn" onclick="window.showBlacklistModal(\'blacklist\')">' +
            I18n.t('room.admin.blacklist') +
            '</button>',
          '<button class="admin-btn" onclick="window.showBlacklistModal(\'whitelist\')">' +
            I18n.t('room.admin.whitelist') +
            '</button>',
          '</div></div>',
          '</div>',
        ].join('');
      ...
      var otherRoomsContent =
        (currentOtherRooms || []).length > 0
          ? currentOtherRooms
              .slice(0, 5)
              .map(function (r) {
                var serverBadge = r.isRemote
                  ? '<span class="other-room-server-badge remote">' +
                    (r.serverName || 'Remote') +
                    '</span>'
                  : '';
                return (
                  ...
                  r.id +
                  (r.isRemote ? '&remote=1' : '') +
                  '" class="other-room-item' +
                  (r.isRemote ? ' other-room-remote' : '') +
                  '"><span class="other-room-name">' +
                  (r.name || 'Room') +
                  serverBadge +
                  '</span><span class="other-room-count">' +
                  r.playerCount +
                  '/' +
                  r.maxPlayers +
                  '</span></a>'
              ...
              .join('')
          : '<div style="padding:20px; text-align:center; color:var(--text-tertiary); font-size:0.85rem; font-style:italic;">' +
            I18n.t('room.no_other_rooms') +
            '</div>';
      ...
      roomDetails.innerHTML = [
        ...
        '<p><strong>' +
          I18n.t('room.mode') +
          ':</strong> <span class="room-mode-tag">' +
          roomMode +
          '</span></p>',
        '<p><strong>' +
          I18n.t('room.host') +
          ...
          (host ? host.name : '') +
          '">' +
          (host ? host.name : I18n.t('common.unknown')) +
          '</span></p>',
        '<p><strong>' +
          I18n.t('room.players') +
          ':</strong> ' +
        ...
        '<p><strong>' +
          I18n.t('room.status') +
          ':</strong> ' +
          (details.state ? details.state.type : 'Unknown') +
          '</p>',
        '<p><strong>' +
          I18n.t('room.locked') +
          ':</strong> <span class="' +
        ...
        '<div class="detail-card" id="card-chart-info"><h3>' +
          I18n.t('room.chart_info') +
          '</h3>' +
          chartInfoHtml +
          '</div>',
