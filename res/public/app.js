...
function navigateTo(page, silent) {
  ...
  document.querySelectorAll('.page-content').forEach(function (c) {
  ...
  if (!silent) {
    var path =
      Object.keys(routeMap).find(function (k) {
        return routeMap[k] === page;
      }) || '/';
...
function updateUserMenu() {
  ...
  if (currentUser) {
    ...
    var placeholder =
      'data:image/svg+xml,' +
      encodeURIComponent(
        '<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40"><rect width="40" height="40" rx="8" fill="#2d2d3a"/><text x="20" y="27" text-anchor="middle" fill="#666" font-size="18" font-family="sans-serif">' +
    ...
    menu.innerHTML =
      '<img src="' +
      (avUrl || placeholder) +
      '" class="user-avatar" onclick="toggleUserDropdown()" onerror="this.onerror=null;this.src=\'' +
      placeholder +
      '\'">' +
      ...
      (isAdmin || isOwner
        ? '<a href="/panel"><svg width="16" height="16" viewBox="0 0 16 16"><circle cx="8" cy="8" r="2.5" fill="none" stroke="currentColor" stroke-width="1.3"/><path d="M8 1 L8 4 M8 12 L8 15 M1 8 L4 8 M12 8 L15 8" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg><span>Admin</span></a>'
        : '') +
    ...
    menu.innerHTML =
      '<button class="login-btn" id="login-trigger" onclick="openAuthModal()">Login</button>';
    document.querySelectorAll('.nav-item[data-page="admin"]').forEach(function (el) {
...
function notify(msg) {
  ...
  n.style.cssText =
    'position:fixed;top:80px;right:24px;background:var(--primary);color:#fff;padding:12px 24px;border-radius:var(--radius);box-shadow:var(--shadow-lg);z-index:3000;font-weight:600;animation:slideIn 0.3s ease;';
  n.textContent = msg;
...
function setupThemeToggle() {
  ...
  var moon =
    '<circle cx="8" cy="8" r="5" fill="none" stroke="currentColor" stroke-width="1.2"/><path d="M8 1 A7 7 0 0 0 8 15 A5 5 0 0 1 8 1" fill="currentColor" opacity="0.3"/>';
  var sun =
    '<circle cx="8" cy="8" r="4" fill="currentColor"/><path d="M8 1 L8 3 M13 3 L11.5 4.5 M14 8 L12 8 M13 13 L11.5 11.5 M8 14 L8 12 M3 13 L4.5 11.5 M1 8 L3 8 M3 3 L4.5 4.5" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/>';
...
function loadServerConfig() {
  ...
  if (b) b.textContent = serverName;
  var s = document.getElementById('server-name-display');
  if (s) s.textContent = serverName;
  var h = document.getElementById('hero-welcome');
  if (h) h.textContent = 'Welcome to ' + serverName;
...
async function loadHomeData() {
  try {
    ...
    var status = await _a[0].json();
    var version = await _a[1].json();
    var a = document.getElementById('online-count');
    if (a) a.textContent = status.onlinePlayers || 0;
    var b = document.getElementById('room-count');
    if (b) b.textContent = status.roomCount || 0;
    var c = document.getElementById('server-version');
    if (c) c.textContent = version.version || '--';
  } catch (e) {
    console.error(e);
  }
}
...
async function loadRooms() {
  ...
  try {
    ...
    if (st) {
      ...
      st.className = 'connection-status connected';
    }
    list.innerHTML = '';
    if (data.rooms && data.rooms.length) {
      ...
      data.rooms.forEach(function (room) {
        ...
        var tag = room.isRemote
          ? '<span class="room-server-tag room-server-remote">' +
            (room.nodeName || 'Remote') +
            '</span>'
          : '';
        card.innerHTML =
          '<h2>#' +
          ...
          '<p>Mode: <span class="room-mode-tag">' +
          (room.cycle ? 'Cycle' : 'Normal') +
          '</span></p>' +
          '<p>Name: <span>' +
          (room.name || '--') +
          '</span></p>' +
          ...
          '</span></p>' +
          '<p>Status: <span>' +
          ((room.state && room.state.type) || 'Idle') +
          '</span></p>' +
          '<p>Locked: <span class="' +
          lockClass +
          '">' +
    ...
    if (st) {
      ...
      st.className = 'connection-status disconnected';
    }
  }
}
      ...
      players.forEach(function (p) {
        ...
        li.innerHTML =
          ...
          (p.avatar ||
            'data:image/svg+xml,' +
              encodeURIComponent(
                '<svg xmlns="http://www.w3.org/2000/svg" width="36" height="36"><rect width="36" height="36" rx="8" fill="#2d2d3a"/><text x="18" y="24" text-anchor="middle" fill="#666" font-size="16" font-family="sans-serif">' +
                  (p.name ? p.name.charAt(0).toUpperCase() : '?') +
                  '</text></svg>',
              )) +
          '" class="player-avatar-small" onerror="this.onerror=null">' +
          '<div><span class="name-prefix ' +
          pfxClass +
          '">' +
          pfxText +
...
async function kickPlayer(userId) {
  ...
  try {
    var res = await fetch('/api/admin/kick-player', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function loadProfile() {
  ...
  function applyProfile() {
    ...
    if (nameEl)
      nameEl.textContent = (userProfile && userProfile.name) || currentUser.name || 'Unknown';
    if (idEl) idEl.textContent = 'ID: ' + currentUser.id;
    if (emailEl)
      emailEl.textContent = (userProfile && userProfile.email) || currentUser.email || '';
    if (rksEl && userProfile && userProfile.rks !== undefined)
      rksEl.textContent = Number(userProfile.rks).toFixed(2);
  }
  ...
  if (avatar) {
    var placeholder =
      'data:image/svg+xml,' +
      encodeURIComponent(
        '<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120"><rect width="120" height="120" rx="20" fill="#2d2d3a"/><text x="60" y="78" text-anchor="middle" fill="#666" font-size="48" font-family="sans-serif">' +
          (currentUser.name ? currentUser.name.charAt(0).toUpperCase() : '?') +
          '</text></svg>',
      );
  ...
  if (roleEl) {
    ...
    roleEl.style.background = roleColor;
  }

  try {
    var sRes = await fetch('/check-session');
    var sData = await sRes.json();
    if (sData.valid) {
      var lt = document.getElementById('session-login-time');
      if (lt) lt.textContent = new Date(sData.expiresAt - 30 * 86400000).toLocaleString();
      var et = document.getElementById('session-expire-time');
      if (et) et.textContent = new Date(sData.expiresAt).toLocaleString();
      var ip = document.getElementById('session-ip');
      if (ip) ip.textContent = sData.ip || '--';
    }
  } catch (e) {
    console.error(e);
  }
