...
interface WebDashboardPluginConfig {
  displayIp?: string;
...
class WebDashboardPlugin {
  ...
  private readonly runtimeIconFile = path.join(process.cwd(), 'icon.png');
  private readonly bundledDefaultIconFile = path.join(__dirname, '../../icon.png');
  ...
  private cleanupLoginAttemptsAndRateLimits(): void {
    const now = Date.now();
  ...
  private verifyUserRole(minRole: 'Admin' | 'Owner'): express.RequestHandler {
    return (req: express.Request, res: express.Response, next: express.NextFunction) => {
      ...
      if (!session || Date.now() > session.expiresAt) {
        if (session) this.userSessions.delete(token);
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/icon.png', (_req, res) => {
      if (fs.existsSync(this.runtimeIconFile)) {
  ...
  private serveHtmlWithConfig(res: express.Response, htmlPath: string): void {
    try {
...
let instance: WebDashboardPlugin | undefined;
